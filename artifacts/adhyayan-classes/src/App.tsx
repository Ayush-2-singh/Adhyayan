...
interface Gallery  { id: string; image_url: string; caption: string; sort_order: number; }
...
function HeroSection() {
  ...
  useEffect(() => {
    ...
    function spawnWord() {
      ...
      const startX = 30+Math.random()*40, startY = 30+Math.random()*20;
      const angle = (Math.random()-0.5)*60;
      const dx = (Math.random()-0.4)*300+"px", dy = -(Math.random()*250+100)+"px";
  ...
  return (
    <section className="ac-hero" id="home" aria-label="Adhyayan Classes Hero">
      ...
      <div style={{ position:"absolute",top:0,right:0,width:200,height:200,pointerEvents:"none" }}>
        {([["#E8651A",0.5,48,30],[,"#F4A460",1.0,38,24],["#E8651A",1.5,32,20]] as [string,number,number,number][]).map(([fill,delay,w,h],i) => (
          <svg key={i} className="ac-bird" width={w} height={h} viewBox="0 0 48 30" fill="none"
            style={{ animationDelay:`${delay}s`,top:[30,10,50][i],right:[20,60,0][i],position:"absolute" }}>
            <path className="ac-wing" d="M24 15 C16 8, 4 5, 0 8 C8 10, 16 14, 24 15Z" fill={fill} opacity={i===2?0.7:1}/>
            <path className="ac-wing" d="M24 15 C32 8, 44 5, 48 8 C40 10, 32 14, 24 15Z" fill={fill} opacity={i===2?0.7:1}/>
            <ellipse cx="24" cy="15" rx="4" ry="2.5" fill={fill} opacity={i===2?0.7:1}/>
          </svg>
        ))}
      </div>

      {/* Book Scene — bulb embedded on right page */}
        ...
        <svg style={{ width:340,height:220,position:"absolute",bottom:0,left:0 }} viewBox="0 0 340 210" fill="none">
          <defs>
            <linearGradient id="acCoverL" x1="0" y1="0" x2="1" y2="1">
              <stop offset="0%" stopColor="#22508C"/><stop offset="100%" stopColor="#1B3D6E"/>
            </linearGradient>
            <linearGradient id="acCoverR" x1="1" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#1B3D6E"/><stop offset="100%" stopColor="#142f54"/>
            </linearGradient>
            <linearGradient id="acPageL" x1="0" y1="0" x2="1" y2="0">
              <stop offset="0%" stopColor="#e8e0d0"/><stop offset="100%" stopColor="#f7f3ec"/>
            </linearGradient>
            <linearGradient id="acPageR" x1="0" y1="0" x2="1" y2="0">
              <stop offset="0%" stopColor="#f7f3ec"/><stop offset="100%" stopColor="#ddd5c2"/>
            </linearGradient>
            <linearGradient id="acSpineGrad" x1="0" y1="0" x2="1" y2="0">
              <stop offset="0%" stopColor="#0d2444"/><stop offset="50%" stopColor="#1B3D6E"/><stop offset="100%" stopColor="#0d2444"/>
            </linearGradient>
          </defs>
          ...
          {/* Page stacks */}
          <rect x="28" y="42" width="138" height="155" rx="3" fill="#ccc5b0" opacity="0.5"/>
          <rect x="30" y="40" width="138" height="155" rx="3" fill="#d4ccba" opacity="0.6"/>
          <rect x="32" y="38" width="138" height="155" rx="3" fill="#ddd5c5" opacity="0.7"/>
          <rect x="174" y="42" width="138" height="155" rx="3" fill="#ccc5b0" opacity="0.5"/>
          <rect x="172" y="40" width="138" height="155" rx="3" fill="#d4ccba" opacity="0.6"/>
          <rect x="170" y="38" width="138" height="155" rx="3" fill="#ddd5c5" opacity="0.7"/>
          ...
          <path d="M112 36 Q170 -5 228 36" stroke="#F4A460" strokeWidth="3" fill="none" opacity="0.6" strokeLinecap="round"
            style={{ strokeDasharray:180,strokeDashoffset:180,animation:"acArcDraw 1.5s ease 1s forwards" }}/>
          ...
          <circle cx="318" cy="60"  r="4" fill="#0d2444" stroke="#E8651A" strokeWidth="1.5"/>
          <circle cx="318" cy="110" r="4" fill="#0d2444" stroke="#E8651A" strokeWidth="1.5"/>
          <circle cx="318" cy="160" r="4" fill="#0d2444" stroke="#E8651A" strokeWidth="1.5"/>

          {/* ── BULB on right page ── */}
          {/* Glow aura */}
          <circle cx="248" cy="108" r="32" fill="rgba(255,200,80,0.10)" className="bulb-glow"/>
          <circle cx="248" cy="108" r="20" fill="rgba(255,200,80,0.08)" className="bulb-glow"/>
          {/* Rays */}
          <line x1="248" y1="70"  x2="248" y2="62"  stroke="#FFB347" strokeWidth="2"   strokeLinecap="round" className="bulb-glow"/>
          <line x1="265" y1="76"  x2="270" y2="71"  stroke="#FFB347" strokeWidth="2"   strokeLinecap="round" className="bulb-glow"/>
          <line x1="231" y1="76"  x2="226" y2="71"  stroke="#FFB347" strokeWidth="2"   strokeLinecap="round" className="bulb-glow"/>
          <line x1="271" y1="93"  x2="278" y2="93"  stroke="#FFB347" strokeWidth="2"   strokeLinecap="round" className="bulb-glow"/>
          <line x1="225" y1="93"  x2="218" y2="93"  stroke="#FFB347" strokeWidth="2"   strokeLinecap="round" className="bulb-glow"/>
          {/* Glass bulb body */}
          <path d="M233 100 C233 88, 263 88, 263 100 C263 109, 258 117, 256 120 L240 120 C238 117, 233 109, 233 100Z"
            className="bulb-glass-on" stroke="rgba(255,180,60,0.7)" strokeWidth="1.2"/>
          {/* Filament */}
          <path d="M240 116 L240 109 Q244 105 248 109 Q252 105 256 109 L256 116"
            className="bulb-filament-on" stroke="#FFD580" strokeWidth="1.8" strokeLinecap="round" fill="none"/>
          {/* Base segments */}
          <rect x="241" y="120" width="14" height="3"  rx="1" fill="rgba(255,255,255,0.35)"/>
          <rect x="243" y="123" width="10" height="2.5" rx="1" fill="rgba(255,255,255,0.25)"/>
          <rect x="244" y="125.5" width="8"  height="2"  rx="1" fill="rgba(255,255,255,0.18)"/>
      ...
      <div style={{ textAlign:"center",animation:"acTextEntry 1.4s cubic-bezier(.22,1,.36,1) 0.3s both" }}>
        ...
        <div style={{ fontFamily:"'Nunito',sans-serif",fontSize:"clamp(1.2rem,3vw,1.8rem)",fontWeight:600,color:"#1B3D6E",letterSpacing:"0.35em",marginTop:12,opacity:0.75,textTransform:"uppercase" }}>Classes</div>
      </div>
      ...
      <a className="ac-cta-btn" href="#contact" onClick={e=>{ e.preventDefault(); scrollTo("contact"); }} aria-label="Enroll Now">
        Enroll Now &nbsp;→
      </a>
...
function Navbar() {
  ...
  function scrollTo(id: string) { document.getElementById(id)?.scrollIntoView({ behavior:"smooth" }); setOpen(false); }
  return (
    <nav style={{ display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 40px",borderBottom:"1.5px solid #eee",position:"sticky",top:0,background:"#fff",zIndex:100,boxShadow:"0 2px 12px rgba(0,0,0,0.06)" }} role="navigation" aria-label="Main navigation">
      ...
      <ul style={{ listStyle:"none",display:"flex",alignItems:"center",gap:22,margin:0,padding:0 }}>
        {[["courses","Courses"],["why","Why Us"],["faculty","Faculty"],["gallery","Gallery"]].map(([id,label])=>(
          <li key={id}>
            <a href={`#${id}`} onClick={e=>{ e.preventDefault(); scrollTo(id); }}
              ...
              onMouseOver={e=>(e.currentTarget.style.color="#1a237e")} onMouseOut={e=>(e.currentTarget.style.color="#333")}>
              {label}
            </a>
          </li>
        ))}
        <li><a href="#contact" onClick={e=>{ e.preventDefault(); scrollTo("contact"); }}
          style={{ background:"#1a237e",color:"#fff",padding:"8px 18px",borderRadius:4,textDecoration:"none",fontWeight:700,fontSize:"0.9rem" }}>
          Enquire Now
        </a></li>
        <li><a href="/admin.html" style={{ color:"#bbb",fontSize:"0.78rem",fontWeight:400,textDecoration:"none" }}>Admin</a></li>
      </ul>
    </nav>
  );
}
...
function CoursesSection({ courses }: { courses: Course[] }) {
  ...
  const badgeColors: Record<string,string> = { "Pre-Foundation":"#e8eaf6", "Foundation":"#e3f2fd", "Commerce":"#e8f5e9" };
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(280px, 1fr))",gap:20 }}>
        {display.map(c=>(
          <article key={c.id} style={{ border:"1.5px solid #ddd",borderRadius:10,padding:"24px 20px",transition:"all 0.2s",cursor:"default",background:"#fff" }}
            ...
            <span style={{ display:"inline-block",background:badgeColors[c.badge]||"#e8eaf6",color:"#1a237e",fontSize:"0.72rem",fontWeight:700,padding:"3px 10px",borderRadius:20,marginBottom:10,textTransform:"uppercase",letterSpacing:"0.5px" }}>{c.badge}</span>
...
function FacultySection({ faculty }: { faculty: Faculty[] }) {
  const defaultFaculty: Faculty[] = [
    ...
    { id:"f2", name:"Faculty Member", subject:"Physics",   experience:"8+ Years Experience",  previous_org:"Senior PGT, More than 8 Years of Experience",  sort_order:1 },
    { id:"f3", name:"Faculty Member", subject:"Chemistry", experience:"10+ Years Experience", previous_org:"Senior PGT, More than 10 Years of Experience", sort_order:2 },
    { id:"f4", name:"Faculty Member", subject:"Biology",   experience:"12+ Years Experience", previous_org:"Senior PGT, More than 12 Years of Experience", sort_order:3 },
  ];

  const display = faculty.length > 0 ? faculty : defaultFaculty;
  const founder = display.find(f => f.sort_order === 0 || f.name.toLowerCase().includes("nitesh") || (f.subject.toLowerCase()==="mathematics" && f.sort_order===0));
        ...
        <div style={{ marginBottom:32 }}>
          <div style={{ background:"linear-gradient(135deg, #1B3D6E 0%, #22508C 100%)",borderRadius:16,padding:"32px 40px",display:"flex",gap:32,alignItems:"center",flexWrap:"wrap",boxShadow:"0 8px 40px rgba(27,61,110,0.25)" }}>
            <div style={{ position:"relative",flexShrink:0 }}>
              <img src="/founder.jpg" alt={`${founder.name} – Founder, Adhyayan Classes`}
                style={{ width:140,height:140,borderRadius:"50%",objectFit:"cover",display:"block" }} className="founder-highlight"/>
              <div style={{ position:"absolute",bottom:-4,right:-4,background:"#E8651A",borderRadius:"50%",width:36,height:36,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.1rem" }} title="Founder">⭐</div>
            </div>
            <div style={{ flex:1,minWidth:200 }}>
              ...
              <div style={{ color:"rgba(255,255,255,0.85)",fontSize:"0.9rem",lineHeight:1.6 }}>
                {founder.previous_org || "Ex Faculty – Aakash Institute & Senior PGT, More than 10 Years of Experience"}
              </div>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit, minmax(200px, 1fr))",gap:20 }}>
          {others.map(f=>(
            <div key={f.id} style={{ border:"1.5px solid #eee",borderRadius:10,padding:"24px 16px",textAlign:"center",transition:"all 0.2s",background:"#fff" }}
              ...
              {f.experience && <div style={{ fontSize:"0.75rem",color:"#555",marginBottom:4 }}>{f.experience}</div>}
              {f.previous_org && <p style={{ fontSize:"0.75rem",color:"#888" }}>{f.previous_org}</p>}
            </div>
          ))}
        </div>
      )}
...
function GallerySection({ gallery }: { gallery: Gallery[] }) {
  const defaultGallery = [
    { id:"g1", image_url:"/founder.jpg", caption:"Founder – Nitesh Kumar Pandey, Adhyayan Classes Varanasi", sort_order:1 },
    { id:"g2", image_url:"/logo.png",    caption:"Dream and Achieve – Adhyayan Classes, Varanasi",          sort_order:2 },
  ];
  const display = gallery.length > 0 ? gallery : defaultGallery;
    ...
    <section id="contact" style={{ maxWidth:1100,margin:"0 auto",padding:"64px 40px" }} aria-labelledby="contact-heading">
      ...
      <div style={{ display:"flex",gap:50,flexWrap:"wrap" }}>
        ...
        <form style={{ flex:2,minWidth:260 }} onSubmit={handleSubmit} aria-label="Enquiry form">
          ...
          {status==="error" && <div style={{ marginTop:12,color:"#c62828",fontWeight:600,fontSize:"0.9rem" }}>❌ Something went wrong. Please call us directly.</div>}
        </form>
      </div>
    </section>
  );
...
export default function App() {
  ...
  useEffect(() => {
    ...
    sbGet("gallery").then(setGallery).catch(()=>setGallery([]));
  }, []);
  ...
  return (
    <>
      ...
      <a href="https://wa.me/919140913384?text=Hello%2C%20I%20am%20interested%20in%20joining%20Adhyayan%20Classes"
        target="_blank" rel="noopener noreferrer" className="wa-float" aria-label="Chat on WhatsApp">
